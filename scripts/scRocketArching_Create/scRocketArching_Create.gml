function scRocketArching_Create() {
	...
	if (rocket_map[? "spawn_extra"])
		for (var i = 1; i <= rocket_map[? "spawn_rockets"]; i++)
			scSpawnRocket(x + (5 * i), y, depth, direction, owner, weapon_map, ["spawn_extra", "spawn_hei", "spawn_len"], [false, map[? "height"] + (1.2 * i), map[? "length"] + (0.3 * i)]);
