...
function scBombGet(argument0) {
	...
	switch (shur) {
		...
		case WEAPON_BOMB_TYPE.ELECTRIC:
			...
			map[? BOMB_MAP.SPEED] = 40;
			map[? BOMB_MAP.TIMER_ACCEL] = 0.5;
			map[? BOMB_MAP.DAMAGE_TYPE] = DAMAGE_TYPE.NONE;
			map[? BOMB_MAP.DAMAGE_ELEMENT] = DAMAGE_ELEMENT.SHOCK;
			map[? BOMB_MAP.WEIGHT] = 20;
		...
		case WEAPON_BOMB_TYPE.PORTAL:
			...
			map[? BOMB_MAP.WEIGHT] = 0;
			map[? BOMB_MAP.TIMER_ACCEL] = 0.8;
			map[? BOMB_MAP.TIMER] = 300;
			map[? BOMB_MAP.DAMAGE] = 15;
			map[? BOMB_MAP.EXPLOSION_SCALE] = 2.5;
