...
//JUMP + F6 = APPLY BUFF
if (global.devmode && global.debug && keyboard_check(scSettingsGetType(SETTINGS_TYPE.VALUE, key_map[? KEYBIND_MAP.JUMP])))
...
if (despawn && ((shootable_map[? SHOOTABLE_MAP.HEALTH_BASE] <= 0 && player_map[? PLAYER_MAP.PLAYER_STATE] == PLAYERSTATE.DEAD && vsp == 0) || image_alpha != 1)) { //Despawn player object
	image_alpha -= 0.025;
	if (image_alpha <= 0)
		instance_destroy();
}