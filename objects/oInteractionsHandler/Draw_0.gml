...
for (var i = 0; i < ds_list_size(ui_boxes); i++) {
	...
	if (i == 0) {
		...
		scDrawText(_x, _y, "Interact debug:", noone, _size, noone, noone, fa_left);
		scDrawText(_x, _y + 20, "msg: " + string(_map[? "msg"]), noone, _size, noone, noone, fa_left);
		scDrawText(_x, _y + 40, "to: " + string(_map[? "to"]), noone, _size, noone, noone, fa_left);
		scDrawText(_x, _y + 60, "from: " + string(_map[? "from"]), noone, _size, noone, noone, fa_left);
		scDrawText(_x, _y + 80, "timer: " + string(_map[? "timer"]), noone, _size, noone, noone, fa_left);
		scDrawText(_x, _y + 100, "alpha: " + string(_map[? "alpha"]), noone, _size, noone, noone, fa_left);
		scDrawText(_x, _y + 125, "amount to go: " + string(ds_list_size(ui_boxes)), noone, _size, noone, noone, fa_left);
	}
}
